...
// QueryUserAuth chains the current query on the "userAuth" edge.
...
// QueryUserMFA chains the current query on the "userMFA" edge.
func (uq *UserQuery) QueryUserMFA() *UserMFAQuery {
	query := (&UserMFAClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// the "userAuth" edge. The optional arguments are used to configure the query builder of the edge.
...
// WithUserMFA tells the query-builder to eager-load the nodes that are connected to
// the "userMFA" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithUserMFA(opts ...func(*UserMFAQuery)) *UserQuery {
	query := (&UserMFAClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserMFA = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withUserAuth; query != nil {
		if err := uq.loadUserAuth(ctx, query, nodes,
			func(n *User) { n.Edges.UserAuth = []*UserAuth{} },
			func(n *User, e *UserAuth) { n.Edges.UserAuth = append(n.Edges.UserAuth, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withUserMFA; query != nil {
		if err := uq.loadUserMFA(ctx, query, nodes,
			func(n *User) { n.Edges.UserMFA = []*UserMFA{} },
			func(n *User, e *UserMFA) { n.Edges.UserMFA = append(n.Edges.UserMFA, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := uq.withUserAuthEvent; query != nil {
		if err := uq.loadUserAuthEvent(ctx, query, nodes,
			...
			func(n *User, e *UserAuthEvent) { n.Edges.UserAuthEvent = append(n.Edges.UserAuthEvent, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadUserAuth(ctx context.Context, query *UserAuthQuery, nodes []*User, init func(*User), assign func(*User, *UserAuth)) error {
	...
	query.Where(predicate.UserAuth(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.UserAuthColumn), fks...))
	}))
...
func (uq *UserQuery) loadUserMFA(ctx context.Context, query *UserMFAQuery, nodes []*User, init func(*User), assign func(*User, *UserMFA)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(usermfa.FieldUserID)
	}
	query.Where(predicate.UserMFA(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.UserMFAColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (uq *UserQuery) loadUserAuthEvent(ctx context.Context, query *UserAuthEventQuery, nodes []*User, init func(*User), assign func(*User, *UserAuthEvent)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userauthevent.FieldUserID)
	}
	query.Where(predicate.UserAuthEvent(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.UserAuthEventColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
